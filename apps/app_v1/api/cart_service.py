...
class CartService:
    ...
    def update_items_in_cart(self, cart):
        ...
        for item in self.cart_items:
            try:
                ...
                if cart_item_db is not None and item['quantity'] <= 0:
                    ...
                    if json_order_item is not None and json_order_item.get('available_quantity') is not None:
                        create_error_response(200, message='Quanity is not Available')
                        return

                    if cart_item_db is not None and (item['quantity'] > cart_item_db.quantity or item['quantity'] < cart_item_db.quantity):
        ...
        try:
            ...
            Logger.error("Error in getting response {%s}" % g.UUID, exc_info=True)
    ...
    def insert_data_to_new_cart(self, request_data, cart):
        ...
        order_item_dict = {}
        response_data = self.fetch_product_price(self.cart_items)
        for response in response_data:
            order_item_dict[response['item_uuid']] = response
        cart_item_list = list()
        for item in self.cart_items:
        ...
        try:
            ...
            Logger.error(str(e), exc_info=True)
            db.session.rollback()
            return create_error_response(200, message='DB Error')
    ...
    def fetch_product_price(self, items):
        ...
        response = requests.post(url=config.DevelopmentConfig.PRODUCT_CATALOGUE_URL, data=json.dumps(data),
                                 headers={'Content-type': 'application/json'})
